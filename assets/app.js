  ...
  openModal(
    ...
    <div class="change-grid">
      <section><b>ИСПРАВЛЕНО</b><ul>
        ...
        <li>Счётчик этапов приведён к фактическим 12 этапам.</li>
      </ul></section>
      ...
      <section><b>ОСТАЛОСЬ</b><ul>
        <li>BeamNG.drive 0.41 — целевой профиль.</li>
...
function showSettings() {
  ...
  openModal(
    `<span class="eyebrow">НАСТРОЙКИ</span><h2>Сетевой бюджет и персонализация</h2>
    ...
    <div class="network-modal-grid">
      <button class="network-card ${state.networkProfile==='standard'?'active':''}" data-modal-network="standard" type="button"><strong>Стандарт</strong><span>2 MB/s</span><small>минимальный расход</small></button>
      ...
      <button class="network-card ${state.networkProfile==='aggressive'?'active':''}" data-modal-network="aggressive" type="button"><strong>Агрессивный</strong><span>20 MB/s</span><small>максимум скорости</small></button>
    </div>
    <p class="micro">Профиль независим от Стандартного/Среднего/Агрессивного режима ремонта. Он задаёт только допустимый сетевой расход для передачи и фоновых запросов.</p>`,
  ...
  $('sSave').addEventListener('click', () => {
    ...
    saveDraft();
    closeModal();
    toast(`Настройки сохранены · ${state.networkMbps} MB/s`, 'success', 2500);
  });
}
...
function beginVisual() {
  ...
  els.stageCount.textContent = `0 / ${state.stages.length || 15}`;
...
function renderResult(report) {
  ...
  if(els.resultMode) els.resultMode.innerHTML=`<span class="mode-result">${report.task_mode==='modify'?'Mod Lab':'Проверка'} · ${esc(report.repair_mode_label||report.repair_mode||state.repairMode)} · target ${esc(report.beamng_version||'0.41')}</span>`;
...
async function loadHealth() {
  try {
    ...
    if (els.versionBadge) els.versionBadge.textContent = `${state.release} · BeamNG ${d.beamng_version || '0.41'}`;
