...
async function startUpload() {
  ...
  xhr.onload = () => {
    ...
    const lastJob = { jobId: d.job_id, fingerprint: fp, sourceName: state.sourceName, kind: state.kind, priority: [...state.priority], wishes: els.wishes.value || '', outputType: out, suffix: state.suffix };
...
async function poll() {
  ...
  try {
    ...
    if (job.status === 'done') {
      ...
      hideGame();
      clearSession();
      saveLastJob({ ...(loadLastJob() || {}), jobId: state.jobId, sourceName: state.sourceName || (loadLastJob()?.sourceName || '') });
      return;
    }
...
function renderResult(report) {
  ...
  saveLastJob({ ...(loadLastJob() || {}), jobId: state.jobId, sourceName: state.sourceName || (loadLastJob()?.sourceName || ''), ts: Date.now() });
...
async function downloadResult() {
  ...
  els.downloadBtn.textContent = 'Проверяем файл…';
  try {
    ...
    // Trigger a browser download without navigating away from ModForge.
    const a = document.createElement('a');
    a.href = url;
    a.download = state.downloadName || r.headers.get('X-ModForge-Artifact') || 'ModForge_result';
    a.rel = 'noopener';
    a.style.display = 'none';
    document.body.appendChild(a);
    a.click();
    setTimeout(() => a.remove(), 1500);
    els.downloadBtn.textContent = 'Скачивание запущено';
    toast('Скачивание запущено — страницу можно не закрывать.', 'success', 3000);
    setTimeout(() => { els.downloadBtn.textContent = original; els.downloadBtn.disabled = false; }, 2200);
  } catch (e) {
...
els.reportBtn?.addEventListener('click', async e => {
  ...
  try {
    ...
    const blob = await r.blob(); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `${state.downloadName || 'ModForge'}_report.json`; a.click(); setTimeout(()=>URL.revokeObjectURL(a.href), 3000);
  } catch (e) { const raw=String(e.message||e); const code=raw.match(/MF-[A-Z0-9-]+/)?.[0] || 'MF-404'; showArtifactError(code, raw.replace(`${code}: `,'')); }
});
