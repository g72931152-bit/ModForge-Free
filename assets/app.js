...
async function downloadResult() {
  ...
  const url = state.downloadUrl || `/api/jobs/${state.jobId}/download`;
  try {
    const r = await fetch(url, { method: 'HEAD', cache: 'no-store' });
    if (!r.ok) {
      let d = {};
      try { d = await r.json(); } catch {}
      throw new Error(`${d.code || `MF-${r.status}`}: ${d.message || 'Готовый файл сейчас недоступен.'}`);
    }
    window.location.assign(url);
  } catch (e) {
    const raw = String(e.message || e);
    const code = raw.match(/MF-[A-Z0-9-]+/)?.[0] || 'MF-404';
    showArtifactError(code, raw.replace(`${code}: `, '') || 'Готовый файл сейчас недоступен.');
  }
}
...
function restartLastJob() {
  if (!state.lastRun || !state.files.length) {
    toast('Для перезапуска нужен текущий выбранный файл. Выберите его заново.', 'warning', 5000);
    resetUI();
    return;
  }
  clearSession();
  state.kind = state.lastRun.kind;
  state.sourceName = state.lastRun.sourceName;
  state.priority = new Set(state.lastRun.priority || []);
  els.wishes.value = state.lastRun.wishes || '';
  els.outputType.value = state.lastRun.outputType || 'zip';
  state.suffix = state.lastRun.suffix || state.suffix;
  els.suffixPreview.textContent = state.suffix;
  setPickerVisual(state.kind);
  saveDraft();
  els.restartJobBtn.hidden = true;
  els.startBtn.disabled = false;
  toast('Перезапускаем задачу…', 'info', 1800);
  startUpload();
}
...
async function feedbackApi(url, opts = {}) {
  const r = await fetch(url, { ...opts, headers: { 'Content-Type': 'application/json', ...(opts.headers || {}) }, cache: 'no-store' });
...
function feedbackHtml(items) {
  ...
  return items.map(item => `
    <article class="feedback-item" data-feedback-id="${esc(item.id)}">
      ...
      ${(item.replies || []).length ? `<div class="feedback-replies">${item.replies.map(r => `<div class="feedback-reply-item"><b>${esc(r.name || 'Гость')}</b><span>${esc(formatDate(r.created_at))}</span><p>${esc(r.message || '')}</p></div>`).join('')}</div>` : ''}
...
async function openFeedback() {
  openModal(`<span class="eyebrow">ОБРАТНАЯ СВЯЗЬ</span><h2>Сообщения и ответы</h2>
    <p class="modal-lead">Сообщайте об ошибках, предлагайте улучшения и отвечайте другим пользователям.</p>
    <div class="feedback-form"><input class="field" id="feedbackName" maxlength="40" placeholder="Ваше имя (необязательно)"><textarea class="field" id="feedbackMessage" maxlength="1200" rows="4" placeholder="Опишите проблему или предложение"></textarea><div class="feedback-form-actions"><button class="btn primary" id="feedbackSend" type="button">Отправить сообщение</button><span id="feedbackHint" class="micro"></span></div></div>
  ...
  $('feedbackSend').addEventListener('click', async () => {
    ...
    try {
      await feedbackApi('/api/feedback', { method: 'POST', body: JSON.stringify({ name: $('feedbackName').value.trim(), message }) });
