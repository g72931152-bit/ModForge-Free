...
STAGES = [
    ...
    ("vehicle", "Автомобиль", "Проверяем колёса, JBeam-связи и очевидные физические аномалии.", 9),
...
def restore_persisted_jobs():
    for meta_path in WORK_ROOT.glob("*/job.json"):
        try:
            ...
            if status not in {"done", "error", "cancelled", "rebuilding"}: continue
...
def send_owner_email(subject: str, body: str) -> None:
    """Best-effort SMTP notification. Feedback is always persisted even when SMTP is not configured."""
...
def make_progress(job: dict, stage_index: int, stage_done: float) -> int:
    ...
    # ETA is based on the actual overall weighted progress, not the current stage's 0-100.
    # Smooth the estimate so stalls near a stage boundary do not claim an unrealistically
    # small number such as 13 seconds while the backend is still working through a heavy file.
    progress_ratio = max(0.01, (completed + weight * max(0.0, min(1.0, stage_done))) / TOTAL_WEIGHT)
    if progress_ratio >= 0.03 and elapsed >= 2.0:
        remaining = elapsed * (1.0 - progress_ratio) / progress_ratio
        previous = job.get("eta_seconds")
        if isinstance(previous, (int, float)) and previous > 0:
            remaining = 0.65 * float(previous) + 0.35 * remaining
        job["eta_seconds"] = max(1, int(round(remaining))) if progress < 100 else 0
    elif progress < 100:
        job["eta_seconds"] = None
...
def scan_vehicle_health(root: Path, job: dict) -> list[dict]:
    ...
    issues=[]; wheel_tokens=0; incomplete=[]; physics=[]; broken_refs=[]; zero_axes=[]
    for p in jbeams:
        text=read_text(p)
        if re.search(r'"pressureWheels"\s*:',text,re.I):
            wheel_tokens += len(re.findall(r'"(?:wheelDir|wheelAxis|node1|node2|wheel|hub|brakeTorque)"\s*:',text,re.I))
            refs=_wheel_refs(text); tokens=_jbeam_string_tokens(text)
            if not refs: incomplete.append(p.relative_to(root).as_posix())
            for wheel,n1,n2 in refs:
                for side,ref in (("node1",n1),("node2",n2)):
                    if ref and ref not in tokens: broken_refs.append(f'{p.relative_to(root).as_posix()}: {wheel}.{side} → {ref}')
            if re.search(r'"(?:wheelDir|wheelAxis)"\s*:\s*\[\s*0\s*,\s*0\s*,\s*0\s*\]',text,re.I): zero_axes.append(p.relative_to(root).as_posix())
        if re.search(r'"(?:beamStrength|beamDeform|nodeWeight|nodeWeightFactor)"\s*:\s*-?0+(?:\.0+)?',text,re.I): physics.append(p.relative_to(root).as_posix())
    if wheel_tokens: issues.append({'level':'ok','stage':'vehicle','title':'Колёсная секция автомобиля обнаружена','details':f'JBeam-файлов: {len(jbeams)}; признаков параметров колёс: {wheel_tokens}.'})
    if incomplete: issues.append({'level':'warning','stage':'vehicle','title':'Подозрение на неполную настройку колёс','details':', '.join(incomplete[:12])})
    if broken_refs: issues.append({'level':'warning','stage':'vehicle','title':'Обнаружены ссылки колёс на неизвестные узлы','details':'; '.join(broken_refs[:16])})
    if zero_axes: issues.append({'level':'warning','stage':'vehicle','title':'Обнаружен нулевой вектор направления колеса','details':', '.join(zero_axes[:12])})
    if physics: issues.append({'level':'warning','stage':'vehicle','title':'Обнаружены нулевые параметры физики','details':', '.join(physics[:12])})
    return issues
...
def inject_modforge_status(root: Path, job: dict) -> None:
    ...
    d=root/'ui'/'modules'/'apps'/'modforgeApplied'; d.mkdir(parents=True,exist_ok=True)
    vehicle_ids=_vehicle_identity_tokens(root)
    info={'name':'ModForge Status','author':'ModForge','version':'0.31','description':'Прозрачный логотип ModForge: показывает, что активен исправленный автомобиль из этого результата.','directive':'modforgeApplied','domElement':'<modforge-applied></modforge-applied>','css':{'width':'78px','height':'58px','top':'18px','right':'18px'},'preserveAspectRatio':True}
    (d/'app.json').write_text(json.dumps(info,ensure_ascii=False,indent=2),'utf-8')
    if (ASSET_ROOT/'logo.png').exists(): shutil.copy2(ASSET_ROOT/'logo.png',d/'app.png')
    (d/'app.js').write_text("angular.module('beamng.apps').directive('modforgeApplied',function(){return{template:'<div class=\"modforge-status\" ng-class=\"{inactive:!active}\" title=\"ModForge · исправленный мод\"><img src=\"/ui/modules/apps/modforgeApplied/app.png\" alt=\"ModForge\"></div>',replace:true,restrict:'EA',link:function(scope){scope.active=false;scope.$on('ModForgeApplied',function(_,data){scope.$evalAsync(function(){scope.active=!!(data&&data.active);});});try{if(window.bngApi&&bngApi.engineLua){bngApi.engineLua(\"extensions.load('modforgeApplied')\");}}catch(e){}}}});",'utf-8')
    (d/'app.css').write_text('.modforge-status{width:100%;height:100%;display:flex;align-items:center;justify-content:flex-end;opacity:.92;transition:opacity .28s,transform .28s}.modforge-status.inactive{opacity:0;transform:translateY(-6px) scale(.96);pointer-events:none}.modforge-status img{width:50px;height:50px;object-fit:contain;filter:drop-shadow(0 5px 14px rgba(0,0,0,.45));transition:transform .28s,filter .28s}.modforge-status:hover img{transform:scale(1.05) rotate(-1.5deg);filter:drop-shadow(0 7px 18px rgba(0,0,0,.56))}','utf-8')
    e=root/'lua'/'ge'/'extensions'; e.mkdir(parents=True,exist_ok=True)
    lua_names=', '.join([f'[{json.dumps(v)}]=true' for v in vehicle_ids])
    lua="""local M={}\nlocal repairedVehicles={VEHICLES}\nlocal function vehicleIsRepaired()\n  local ok,veh=pcall(function() return be and be:getPlayerVehicle(0) end)\n  if not ok or not veh then return false end\n  local ok2,filename=pcall(function() return veh:getJBeamFilename() end)\n  if not ok2 or not filename then return false end\n  filename=tostring(filename):gsub('\\\\','/'):lower()\n  filename=filename:match('([^/]+)$') or filename\n  filename=filename:gsub('%.jbeam$','')\n  if repairedVehicles[filename] then return true end\n  for id,_ in pairs(repairedVehicles) do if filename==id or filename:find(id,1,true) then return true end end\n  return false\nend\nlocal function emit() guihooks.trigger('ModForgeApplied',{active=vehicleIsRepaired(),service='ModForge',version='0.31-beta'}) end\nM.onExtensionLoaded=emit\nM.onVehicleSpawned=emit\nM.onVehicleSwitched=emit\nM.onUpdate=function() local t=os.clock(); if not M._next or t>=M._next then M._next=t+0.60; emit() end end\nreturn M\n""".replace('VEHICLES',lua_names)
    (e/'modforgeApplied.lua').write_text(lua,'utf-8')
    marker={'service':'ModForge','version':APP_VERSION,'repair_mode':job.get('repair_mode','standard'),'status':'repaired-artifact','vehicle_status_app':True,'vehicle_ids':vehicle_ids,'status_app_path':'ui/modules/apps/modforgeApplied','activation':'Добавьте ModForge Status один раз через BeamNG → UI Apps. По умолчанию приложение размещается справа сверху и скрывается для других vehicle-моделей.'}
...
async def run_job(job_id: str, root: Path, original_name: str, source_kind: str, source_name: str, wishes: str, priority: list[str], uploaded_bytes: int, output_suffix: str, output_type: str, repair_mode: str="standard") -> None:
    job=jobs[job_id]
    job["repair_mode"]=repair_mode
    try:
        ...
        job["source_file_count"]=count; job["unpacked_bytes"]=unpacked; job["source_kind"]=source_kind; job["source_name"]=source_name; job["original_name"]=original_name; job["output_suffix"]=output_suffix; job["output_type"]=output_type; persist_job(job_id)
        ...
        stage_start(job,5,"Проверяем колёса, JBeam-связи и очевидные физические аномалии."); issues.extend(await asyncio.to_thread(scan_vehicle_health,payload,job))
        stage_start(job,6,"Ищем дополнительные проблемы, которые требуют особого внимания."); issues.extend(await asyncio.to_thread(scan_deep,payload,job))
        stage_start(job,7,f"Применяем режим: {REPAIR_MODES[repair_mode]['label']}."); repairs=await asyncio.to_thread(repair_tree,payload,job); issues.extend(repairs); fixed=sum(1 for i in repairs if i.get("level")=="fixed")
        stage_start(job,8,"Проверяем изменённые файлы сразу после ремонта."); issues.extend(await asyncio.to_thread(verify_tree,payload,job,8))
        stage_start(job,9,"Повторно запускаем полный контроль после изменений."); issues.extend(await asyncio.to_thread(verify_tree,payload,job,9))
        focus=focus_from(priority,wishes)
        if source_kind=="single" and output_type=="same":
            original=next(p for p in payload.iterdir() if p.is_file()); out_name=output_name(source_name or original_name,output_suffix,False); out_path=root/out_name; shutil.copy2(original,out_path); make_progress(job,10,1)
        else:
            stage_start(job,10,"Собираем результат с сохранением структуры мода и ModForge-индикатором."); out_name=output_name(source_name or original_name,output_suffix,True); out_path=root/out_name; await asyncio.to_thread(package_zip,payload,out_path,job)
        stage_start(job,11,"Проверяем итоговый файл перед выдачей.")
...
async def analyze(request: Request, source_kind: str=Form("zip"), source_name: str=Form(""), wishes: str=Form(""), priority_json: str=Form("[]"), files: list[UploadFile]=File(...), manifest_json: str=Form("[]"), output_suffix: str=Form("FIXED"), output_type: str=Form("zip"), repair_mode: str=Form("standard")):
...
async def restart_job(job_id: str):
    ...
    try:
        ...
        jobs[new_id] = {
            ...
            "_cancel_event":threading.Event(), "_pause_event":threading.Event()
        }
        persist_job(new_id)
        asyncio.create_task(run_job(new_id, root, original_name, source_kind, source_name, wishes, priority, uploaded_bytes, output_suffix, output_type, repair_mode))
